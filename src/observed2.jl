...
function (sol::ODESolutionWithStaticCache)(t::Number, ::Type{deriv}, idxs, continuity) where {deriv}
  ...
  interp_sol = sol.interp([t], nothing, deriv, interp_p, continuity)
  observed = SciMLBase.has_observed(sol.prob.f) ? sol.prob.f.observed : SciMLBase.DEFAULT_OBSERVED
  observed(idxs, interp_sol.u[1], interp_p, t)
end
...
function (sol::ODESolutionWithStaticCache)(t::AbstractVector{<:Number}, ::Type{deriv}, idxs, continuity) where {deriv}
  ...
  interp_sol = sol.interp(t, nothing, deriv, interp_p, continuity)
  observed = SciMLBase.has_observed(sol.prob.f) ? sol.prob.f.observed : SciMLBase.DEFAULT_OBSERVED
  observed_sol = [observed(idxs, interp_sol.u[i], static_interpolation(sol,tᵢ,continuity), tᵢ) for (i,tᵢ) in enumerate(t)]
  RecursiveArrayTools.DiffEqArray(observed_sol, t, [idxs], SciMLBase.getindepsym(sol), observed, interp_p)
end
